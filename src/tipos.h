...
typedef struct {				// Tipo = Aluno
	...
	int grade[DIAS][TURNOS];			// Grade de horários [i][j]; i -> dias da semana (seg <-> sex); j -> horarios (m1 <-> t3 <-> n6); Elementos são códigos de disciplinas
} Dados_aluno;
...
typedef struct {				// Tipo = Professor
	...
	int aulas[DIAS][TURNOS]; 			// Grade das aulas por dia e horário; elementos são as salas em que ocorrerão as aulas
} Dados_professor;
...
int BuscaGrade(int grade[DIAS][TURNOS], int elem);				// Checa se um elemento 'elem' já está na grade
